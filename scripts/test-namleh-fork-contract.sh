#!/usr/bin/env bash
set -euo pipefail

repo_root=$(git rev-parse --show-toplevel)
script="$repo_root/scripts/configure-namleh-remotes.sh"
scratch=$(mktemp -d)
trap 'rm -rf "$scratch"' EXIT

git -C "$scratch" init -q
git -C "$scratch" -c core.hooksPath=/dev/null -c user.name=ci -c user.email=ci@example.com commit -q --allow-empty -m baseline
git -C "$scratch" branch -M main
git -C "$scratch" remote add origin https://github.com/Namleh-Studios/buzz.git
git -C "$scratch" remote add upstream https://github.com/example/writable-upstream.git
git -C "$scratch" remote set-url --add upstream https://github.com/example/hidden-fetch.git
git -C "$scratch" remote set-url --push upstream https://github.com/example/writable-upstream.git
git -C "$scratch" remote set-url --add --push upstream https://github.com/example/hidden-push.git
git -C "$scratch" config --unset-all remote.upstream.fetch
git -C "$scratch" config --add remote.upstream.fetch '+refs/heads/*:refs/heads/*'

git -C "$scratch" -c safe.directory="$scratch" -c core.hooksPath=/dev/null status >/dev/null
(
  cd "$scratch"
  "$script" >/dev/null
)

[[ $(git -C "$scratch" remote get-url --all upstream) == "https://github.com/block/buzz.git" ]]
[[ $(git -C "$scratch" remote get-url --push --all upstream) == "/dev/null" ]]
[[ $(git -C "$scratch" config --get-all remote.upstream.fetch) == '+refs/heads/*:refs/remotes/upstream/*' ]]
[[ $(git -C "$scratch" remote get-url --all origin) == "https://github.com/Namleh-Studios/buzz.git" ]]
[[ $(git -C "$scratch" remote get-url --push --all origin) == "https://github.com/Namleh-Studios/buzz.git" ]]

if git -C "$scratch" push --dry-run upstream main >/dev/null 2>&1; then
  echo "upstream accepted a push after the read-only guard was installed" >&2
  exit 1
fi

fresh="$scratch/fresh"
mkdir "$fresh"
git -C "$fresh" init -q
git -C "$fresh" remote add origin ssh://git@github.com/Namleh-Studios/buzz
(
  cd "$fresh"
  "$script" >/dev/null
)
[[ $(git -C "$fresh" remote get-url --all upstream) == "https://github.com/block/buzz.git" ]]
[[ $(git -C "$fresh" remote get-url --push --all upstream) == "/dev/null" ]]

missing_origin="$scratch/missing-origin"
mkdir "$missing_origin"
git -C "$missing_origin" init -q
if (cd "$missing_origin" && "$script" >/dev/null 2>&1); then
  echo "configure-namleh-remotes.sh accepted a repository without origin" >&2
  exit 1
fi

git -C "$scratch" remote set-url --push origin https://github.com/block/buzz.git
if (
  cd "$scratch"
  "$script" >/dev/null 2>&1
); then
  echo "configure-namleh-remotes.sh accepted a non-Namleh origin" >&2
  exit 1
fi

git -C "$scratch" config --unset-all remote.origin.pushurl || true
git -C "$scratch" config --add remote.origin.pushurl https://github.com/Namleh-Studios/buzz.git
git -C "$scratch" config --add remote.origin.pushurl https://github.com/block/buzz.git
if (
  cd "$scratch"
  "$script" >/dev/null 2>&1
); then
  echo "configure-namleh-remotes.sh accepted a hidden non-Namleh origin push URL" >&2
  exit 1
fi

git -C "$scratch" config --unset-all remote.origin.pushurl
git -C "$scratch" config --add remote.origin.pushurl https://github.com/Namleh-Studios/buzz.git
git -C "$scratch" remote set-url origin https://github.com/block/buzz.git
if (
  cd "$scratch"
  "$script" >/dev/null 2>&1
); then
  echo "configure-namleh-remotes.sh accepted a non-Namleh origin fetch URL" >&2
  exit 1
fi

git -C "$scratch" remote set-url origin https://github.com/Namleh-Studios/buzz.git
git -C "$scratch" remote set-url --add origin https://github.com/block/buzz.git
if (
  cd "$scratch"
  "$script" >/dev/null 2>&1
); then
  echo "configure-namleh-remotes.sh accepted a hidden non-Namleh origin fetch URL" >&2
  exit 1
fi

echo "Namleh fork remote contract passed."
