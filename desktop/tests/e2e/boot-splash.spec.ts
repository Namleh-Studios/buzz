...
// loading gate keeps the Namleh mark up as an overlay above the already
...
test("boot splash overlay holds with the Namleh mark, then dismisses", async ({
  page,
}) => {
  ...
  await expect(overlay.getByTestId("namleh-app-mark")).toBeVisible();
  await expect(overlay.locator(".bee-wing-left")).toHaveCount(0);
