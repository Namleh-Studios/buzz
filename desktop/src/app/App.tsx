...
import { NamlehAppMark } from "@/shared/ui/NamlehAppMark";
...
function AppMarkLoader({ className }: { className?: string }) {
  return <NamlehAppMark className={cn("block h-auto w-full", className)} />;
}
...
function AppLoadingGate() {
  return (
    ...
    >
      ...
      <NamlehAppMark className="relative z-10 h-auto w-28" />
    </div>
  );
}
...
function CommunitySwitchGate() {
  ...
  return (
    ...
    >
      ...
      {showSpinner ? <AppMarkLoader className="h-auto w-20" /> : null}
    </div>
  );
}
